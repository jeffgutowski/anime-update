...
<a class="notification hvr-grow-shadow2 {{ $notification->read_at ? 'grayscale' : '' }}" href="{{$offer->url}}" data-notif-id="{{$notification->id}}">
  ...
  <div class="info">
    ...
    <h1>
      @if($notification->data['status'] == 'declined')
        {{ trans('notifications.offer_status_declined', ['username' => $listing->user->name, 'gamename' => $listing->product->name]) }}
      @else
        {{ trans('notifications.offer_status_accepted', ['username' => $listing->user->name, 'gamename' => $listing->product->name]) }}
      @endif
    </h1>
