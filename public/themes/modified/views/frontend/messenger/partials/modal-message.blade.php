        ...
        <div class="selected-user {{ !isset($user) ? 'hidden' : 'flex-center-space' }}" id="selected-user">
          ...
          @if(isset($user))
            ...
            <div class="selected-user-info">
              <span class="avatar @if($user->isOnline()) avatar-online @else avatar-offline @endif m-r-10" style="vertical-align: inherit !important;"><img src="{{ $user->avatar_square_tiny }}" class="img-circle avatar-img-md"><i></i></span><input name="recipient" type="hidden" value="{{ $user->id }}"> {{ $user->name }}
            </div>
          @else
...
$(document).ready(function(){
  ...
  @if(!isset($user))
    ...
    $('#offersearch').typeahead(null, {
      ...
      templates: {
        ...
        suggestion: function (data) {
          return '<div class="searchresult hvr-grow-shadow2"><span class="link"><div class="inline-block m-r-10"><span class="avatar avatar-'+ data.status +'" style="vertical-align: inherit !important;"><img src="' + data.avatar + '" class="img-circle avatar-img-md"><i></i></span></div><strong class="title">' + data.name + '</strong></span></div>';
        }
      }
    })
    ...
    $('#offersearch').bind('typeahead:selected', function(obj, datum, name) {
      $('.selected-user-info').html('<span class="avatar avatar-'+ datum.status +' m-r-10" style="vertical-align: inherit !important;"><img src="' + datum.avatar + '" class="img-circle avatar-img-md"><i></i></span><input name="recipient" type="hidden" value="' + datum.id +'">' + datum.name);
