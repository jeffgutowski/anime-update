...
@section('user-content')
  ...
  <div class="flex-center-space m-b-20">
    <div class="flex-center">
      ...
      @if(auth()->check() && is_null($friend) && $user->id != auth()->id())
        <div class="friend-request-wrapper">
          <button class="btn btn-success btn-friend" onclick="friendAction(this, {{$user->id}}, '/friends/', 'Requested')">Request Friend</button>
        </div>
      @endif
    </div>
  ...
  @if(auth()->check() && auth()->id() == $user->id)
    <div>
        <a href="{{url("/dash/quick-shop")}}" class="wishlist-link">
              <i class="fa fa-store-alt"><span style="font-family: Roboto, sans-serif"> My Quick Shop</span></i>
        </a>
    </div>
  @elseif(auth()->check())
    <div>
      <button id="calc-similarity" class="btn btn-success">Calculate Similarity</button>
      <div id="similarities">
        <div>You own <span id="percent_self_owns_of_friend"></span>% of their collection.</div>
        <div>You have completed <span id="percent_self_played_of_friend"></span>% of games they have completed.</div>
        <div>They own <span id="percent_friend_owns_from_self"></span>% of your collection.</div>
        <div>They have completed <span id="percent_friend_played_from_self"></span>% of games you have completed.</div>
      </div>
    </div>
  @endif
@stop
