...
<div class="flex-center wrap m-b-20">
  ...
  <a href="{{Auth::user()->url."?frag=wishlist#wishlist"}}" class="quick-action">
    {{-- Icon --}}
    <div class="quick-icon">
      ...
      <div class="panel-heading listing-heading">
        <div class="flex-center-space">
          <div class="flex-center">
            ...
            <div class="m-r-10">
              <span class="avatar">
                <img src="{{ $listing->product->image_square_tiny }}" alt="{{ $listing->product->name }}">
              </span>
            </div>
            {{-- Game Name + platform --}}
            <div>
              <div class="title">{{ $listing->product->name }}</div>
              <span class="platform-label" style="background-color:{{ $listing->product->platform->color }};"> {{ $listing->product->platform->name }} </span>
            </div>
          </div>
              ...
              <div class="listing-detail-fix flex-center">
                ...
                <div class="trade-offer-game flex-center">
                  ...
                  <div class="avatar m-r-10">
                    <img src="{{ $offer->product->image_square_tiny }}" alt="{{ $offer->product->name }}">
                  </div>
                  <div>
                    {{-- Game title & platform / icon when suggestion --}}
                    <div class="offer-game-title">@if(!$offer->trade_from_list)<span class="m-r-5"><i class="fa fa-retweet" aria-hidden="true"></i></span>@endif{{ $offer->product->name }}</div>
                    <span class="platform-label" style="background-color:{{ $offer->product->platform->color }};">{{ $offer->product->platform->name }} </span>
                  </div>
                </div>
                @endif
          ...
          <div class="modal-header">
            ...
            <div class="title">
              ...
              <h4 class="modal-title" id="myModalLabel">
                ...
                {{ trans('users.modal_delete_listing.title', ['gamename' => $listing->product->name]) }}
              </h4>
            </div>
        ...
        <div class="panel-heading listing-heading">
          <div class="flex-center-space">
            <div class="flex-center">
              ...
              <div class="m-r-10">
                <span class="avatar">
                  <img src="{{ $offer->listing->product->image_square_tiny }}" alt="{{ $offer->listing->product->name }}">
                </span>
              </div>
              {{-- Game Name + platform --}}
              <div>
                <div class="title">{{ $offer->listing->product->name  }}</div>
                <span class="platform-label" style="background-color:{{ $offer->listing->product->platform->color }};"> {{ $offer->listing->product->platform->name }} </span>
              </div>
            </div>
          </div>
        </div>
            ...
            <div class="listing-detail">
              <div class="listing-detail-fix flex-center">
                ...
                <div class="trade-offer-game flex-center">
                  ...
                  <div>
                    {{-- Game title & platform --}}
                    <div class="offer-game-title">{{ $offer->product->name }}</div>
                    <span class="platform-label" style="background-color:{{ $offer->product->platform->color }};">{{ $offer->product->platform->name }} </span>
                  </div>
                </div>
                @endif
        ...
        <div class="modal-content">
          ...
          <div class="modal-header">
            ...
            <div class="title">
              ...
              <h4 class="modal-title" id="myModalLabel">
                <i class="fa fa-trash" aria-hidden="true"></i>
                {{ trans('users.modal_delete_offer.title', ['gamename' => $offer->listing->product->name]) }}
              </h4>
            </div>
