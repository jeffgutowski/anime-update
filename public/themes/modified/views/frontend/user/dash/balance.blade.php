          ...
          @else
            <div class="value pickup p-10">
              <div class="flex-center">
                ...
                <div class="m-r-10">
                  <span class="avatar">
                    <img src="{{ $transaction->item->listing->product->image_square_tiny }}" alt="{{ $transaction->item->listing->product->name }}">
                  </span>
                </div>
                {{-- Game Name + platform --}}
                <div>
                  <div class="title text-white f-w-500">{{ $transaction->item->listing->product->name }}</div>
                  <span class="platform-label" style="background-color:{{ $transaction->item->listing->product->platform->color }};"> {{ $transaction->item->listing->product->platform->name }} </span>
                </div>
              </div>
            </div>
          @endif
