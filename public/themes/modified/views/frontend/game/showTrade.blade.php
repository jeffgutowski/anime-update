    ...
    <div class="card game-cover-wrapper hvr-grow-shadow">
      ...
      <a href="{{ $listing->url_slug }}">
        ...
        @if($listing->product->cover_generator)
          <div class="lazy-trade game-cover gen"  data-original="{{$listing->product->image_cover}}"></div>
          <div class="game-platform-gen" style="background-color: {{$listing->product->platform->color}}; text-align: {{$listing->product->platform->cover_position}};">
            ...
            @if( config('settings.platform_logo') )
              <img src="{{ asset('logos/' . $listing->product->platform->acronym . '_tiny.png/') }}" alt="{{$listing->product->platform->name}} Logo">
            @else
        ...
        {{-- Normal game cover --}}
        @else
          <div class="lazy-trade game-cover"  data-original="{{$listing->product->image_cover}}"></div>
        @endif
        {{-- Item name --}}
        @if($listing->product->image_cover)
        <div class="item-name">
