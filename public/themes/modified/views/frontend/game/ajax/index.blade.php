  ...
  <div class="m-b-20 flex-center-space">
      ...
      <div>
          ...
          <a id="order-direction" href="{{ url('games/order') }}/{{ session()->has('gamesOrder') ? session()->get('gamesOrder') : 'name' }}/{{  session()->has('gamesOrderByDesc') ? (session()->get('gamesOrderByDesc') ? 'asc' : 'desc') : 'asc' }}" class="btn btn-dark" style="vertical-align: inherit;">
          ...
          <div class="m-l-5 inline-block">
              <select id="order_by" class="form-control select" style="height: 33px !important;">
                  ...
                  {{-- Name --}}
                  <option value="{{ route('games', ['order' => 'name']) }}">{{ trans('general.sortfilter.name') }}</option>
                  {{-- Release option --}}
                  <option value="{{ route('games', ['order' => session('region.code')]) }}">{{ trans('general.sortfilter.sort_release') }}</option>
                  {{-- Listings option --}}
                  <option value="{{ route('games', ['order' => 'listings']) }}">{{ trans('general.sortfilter.sort_listings') }}</option>
              </select>
          </div>
      </div>
      {{-- End sort options --}}
