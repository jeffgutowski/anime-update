...
<html class="no-js css-menubar{{ !Request::is('games') && !Request::is('listings') ? ' overflow-smooth' : '' }}{{ Request::is('messages') ? ' messages' : '' }}" lang="{{config('settings.default_locale')}}">
  <head>
    ...
    <link rel="stylesheet" href="{{ asset('themes/' . Theme::getCurrent() . '/assets/css/site.css') }}?v={{hash_file('md5', base_path().'/public/themes/'.Theme::getCurrent().'/assets/css/site.css')}}">
    <link rel="stylesheet" href="{{ asset('css/custom.css') }}?v={{hash_file('md5', base_path().'/public/css/custom.css')}}">
    ...
    <link rel="stylesheet" href="{{ asset('css/check3s.css') }}">
  ...
  <script src="{{ asset('js/check3s.js') }}?v={{ hash_file('md5', base_path().'/public/js/check3s.js') }}"></script>
  <script src="{{ asset('js/rating.js') }}?v={{ hash_file('md5', base_path().'/public/js/rating.js') }}"></script>
  <script src="{{ asset('js/filters.js') }}?v={{ hash_file('md5', base_path().'/public/js/filters.js') }}"></script>
  ...
  $(document).ready(function(){
    ...
    $('#navbar-search').keypress(function(e) {
      if(e.which == 13){
        ...
        if($('#navbar-search').val() != "")
          window.location.href = {!! '"' . url('/product') . '/?o=-rank&q="'!!} + $('#navbar-search').val();
      }
    });
    ...
    $('#navbar-search').typeahead(null, {
      ...
      templates: {
        ...
        suggestion: function (data) {
            ...
            return '<div class="searchresult navbar"><a href="' + data.url + '"><div class="inline-block m-r-10"><span class="avatar"><img src="' + data.pic + '" class="img-circle"></span></div><div class="inline-block"><strong class="title">' + data.name + '</strong><span class="release-year m-l-5">' + data.release_year +'</span><br><small class="text-uc text-xs"><span class="platform-label" style="background-color: ' + data.platform_color + '; color:'+ data.platform_text_color +'">' + data.platform_name + '</span> ' + listings + ''+ cheapest_listing +'</small></div></a></div>';
        }
      }
    })
    ...
    $(document).click(function (event) {
      ...
      var _opened = $("#site-navbar-search").hasClass("collapse in");
      if (_opened === true && (!clickover.hasClass("gs-search-bar") && !clickover.hasClass("navbar"))) {
        $("#search-close").click();
      }
    });
  });
