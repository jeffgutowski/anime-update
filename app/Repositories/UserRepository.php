...
class UserRepository extends Repository
{
    ...
    public function updateProfile($id, $request)
    {
        ...
        if ($request->hasFile('avatar')) {
            ...
            $destination_path = env('S3_DESTINATION')."users/avatar";
            ...
            if (!is_null($user->avatar)) {
                \Storage::disk($disk)->delete(str_replace(env('S3_BUCKET_URL'), '', $user->avatar));
            }

            $user->avatar = env('S3_BUCKET_URL').$destination_path.'/'.$newfilename;
        }

        $user->about_me = !empty($request['about_me']) ? $request['about_me'] : null;
        $user->favorite_game = !empty($request['favorite_game']) ? $request['favorite_game'] : null;
        $user->favorite_genre_id = !empty($request['favorite_genre_id']) ? $request['favorite_genre_id'] : null;
        $user->favorite_platform_id = !empty($request['favorite_platform_id']) ? $request['favorite_platform_id'] : null;
        $user->favorite_developer = !empty($request['favorite_developer']) ? $request['favorite_developer'] : null;
        $user->favorite_publisher = !empty($request['favorite_publisher']) ? $request['favorite_publisher'] : null;
    ...
    public function updateLocation($id, $request)
    {
        ...
        $country = Country::where('code', $user_location->country_abbreviation)->first();
        $user_location->region_code = $country->region_code;
        $user_location->save();
