...
class OfferController
{
    ...
    public function show($id)
    {
        ...
        SEO::setTitle(trans('general.title.offer', ['page_name' => config('settings.page_name'), 'platform' => $listing->product->platform->name, 'game_name' => $listing->product->name, 'user_name' => $offer->user->name]));

        $trade_game = Game::find($offer->trade_game);

        $thread = Thread::findOrFail($offer->thread_id);

        return view('frontend.offer.show', ['offer' => $offer, 'listing' => $listing, 'game' => $listing->product, 'trade_game' => $trade_game, 'user' => $user, 'thread' => $thread]);
    }
    ...
    public function add(Request $request)
    {
            ...
            if (!is_null($trade_list) && array_key_exists($request->trade_game, $trade_list) && !$request->add_charge_user && !$request->add_charge_partner) {
              ...
              if ($trade_list[$request->trade_game]['price_type'] != 'none') {
                ...
                if ($listing->trade_negotiate) {
                    ...
                    if ($request->trade_game == $listing->product->id) {
        ...
        $thread = Thread::create(
            [
                'subject' => $listing->product->name . " offer from " . \Auth::user()->name,
                'offer_id' => $offer->id
            ]
        );
        ...
        if ($offer->trade_game) {
            ...
            Message::create(
                [
                    ...
                    'body'      => trans('offers.general.chat_trade', ['game_name' => $listing->product->name, 'platform_name' => $listing->product->platform->name, 'trade_game' => $offer->product->name, 'Trade_platform' => $offer->product->platform->name]),
                ]
            );
        } else {
            // Message
            Message::create(
                [
                    'thread_id' => $thread->id,
                    'user_id'   => \Auth::user()->id,
                    'body'      => trans('offers.general.chat_buy', ['game_name' => $listing->product->name, 'platform_name' => $listing->product->platform->name, 'price' => $offer->price_offer_formatted]),
                ]
            );
        }
    ...
    public function delete(Request $request)
    {
        ...
        \Alert::error('<i class="fa fa-trash m-r-5"></i>' . trans('offers.alert.deleted', ['game_name' => str_replace("'", '', $offer->listing->product->name)]))->flash();
    ...
    public function pay($id)
    {
        ...
        $items[] = array(
            'name' => $listing->product->name . ' (' . $listing->product->platform->name . ')',
