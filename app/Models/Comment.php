  ...
  public function getItemAdmin()
  {
      if ($this->fresh()->type == 'game' ) {
          if (isset($this->fresh()->game)) {
              return '<div class="user-block">
                  ...
                  <span class="username"><i class="fa fa-gamepad"></i> <a href="' . $this->fresh()->product->url_slug .'#!comments" target="_blank">' . $this->fresh()->product->name . '</a></span>
      ...
      if ($this->fresh()->type == 'listing' ) {
          if (isset($this->fresh()->listing)) {
              return '<div class="user-block">
                  <img class="img-circle" src="' . $this->fresh()->listing->product->image_square_tiny . '" alt="User Image">
                  <span class="username"><i class="fa fa-tag"></i> <a href="' . $this->fresh()->listing->url_slug .'#!comments" target="_blank">' . $this->fresh()->listing->product->name . '</a></span>
                  <span class="description"><span class="label" style="background-color: '. $this->fresh()->listing->product->platform->color . '; margin-right: 10px;">' . $this->fresh()->listing->product->platform->name .'</span><i class="fa fa-calendar"></i> ' . $this->fresh()->listing->product->release_date->format('Y') . '</span>
              </div>';
          } else {
              return '<div class="user-block text-danger text-bold">
                  <i class="fa fa-ban"></i> Removed
